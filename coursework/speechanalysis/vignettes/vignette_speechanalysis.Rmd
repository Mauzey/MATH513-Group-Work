...
Zipf’s law states that the word frequencies in any word datasets will be very similar to Zipf’s distribution - the number of occurrences of the Nth most frequent word would be X/N, where X is the frequency of the most frequent word. 

Most recent studies of this phenomena show that in the case of words, typically there is the same value of 𝞪, and the frequency on Nth word is described as: X/N^𝞪 However, we couldn't find how 𝞪 is being calculated, therefore we set 𝞪=1 in order to explore the Zipf's law in its standard form.

First, plotting the frequencies of the words in Trump's speeches grouped by their locations and with linear regression line in order to observe the tendency of word frequencies.

```{r zipfs_law, warning=FALSE, message=FALSE, fig.width=10, fig.height=6, out.width="98%"}
zipfs_law(trump_speeches, 2)

```
Linear regression attempts to model the relationship between two variables by fitting a linear equation to observed data. In our case, we're trying to determine a relationship between the word rank and the term frequency.


In general, the 'term frequency - word rank' curve follows the regression line trend.
However, we can see that Trump uses words with ranks 10-100 more frequently than predicted by the linear
regression model. On top of this, the predicted term frequency for the most frequent word (which has rank No1) is supposed to be bigger, which isn't followed by the Trump.
...
In order to explore how much Trump's speeches deviate from the theoretical Zipf's law, we counted the word frequencies for each word across the dataset comprised of all the provided speeches and plotted them alongside the word frequencies calculated using the Zipf's law. The red dash line indicates the 0-point on Y-axis.
As it can be seen,  Trump's word frequencies are overall higher than the predicted ones as the line with observed values declines more slowly, which means that he uses most popular and simple words (in the given speech dataset it's 'the', 'and', 'a', 'you', 'up', 'to' etc) more often than normal (according to Zipf's law). 
## References
...
- Linear Regression. Available at: http://www.stat.yale.edu/Courses/1997-98/101/linreg.htm (Accessed: 13 January 2021)

- Maj, M (2019) Investigating words distribution with R – Zipf’s law. Available at: https://www.r-bloggers.com/2019/02/investigating-words-distribution-with-r-zipfs-law/ (Accessed: 13 January 2021)
