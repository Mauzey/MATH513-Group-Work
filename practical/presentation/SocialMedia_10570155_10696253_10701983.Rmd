...
<!--- SPEAKER NOTES - WILL
  ...
  We utilized RStudio in combination with the 'rTweet' package to extract and analyze the data from the Twitter Developer API.

  To allow us to work professionally and on various parts of the project as a group, we decided to break up the code into 
  sections and utilize GitHub to keep track on the project.
...
<!--- SPEAKER NOTES: - WILL
  ...
  We came to the conclusion that it was better to use hash-tags instead of pulling data from profiles. This was to ensure we would 
  get a wide range of opinions for a fairer analysis of the public's sentiment.

  For our hash-tags, we looked to the Samsung Mobile page and identified that the hash-tags '#GalaxyS20' and '#GalaxyS20FE' were 
  the official hash-tags used by Samsung.
  ...
  We decided to look to the CEO of Apple, Tim Cook, as we thought he would most likely be using the official hash-tag as deemed by 
  Apple's marketing team, which led us to the hash-tag '#iPhone12'.

--->

<!--- EXTRA NOTES (Cut from speaker notes to save time):

  We performed some sample searches of other hash-tags like S20 and S20FE using the advanced Twitter search, on the website, to 
  determine how relevant the hash-tag was.
  ...
  in the list. This identified whether a hash-tag was being used by the public and this further supported our findings that the 
  hash-tags '#iPhone12', '#SamsungS20' and '#SamsungS20FE' were the best to pull data.
...
<!--- SPEAKER NOTES - Will
  ...
  Due to the limitations of the general search, we utilized the full archive premium feature to pull tweets relating to the S20, 
  seen here in red. 
  ...
  We learned from this and corrected our code and pulled as much data as we could for the other two phone using the premium 30 
  day function.
...
<!--- SPEAKER NOTES - Tania
  ...
  model we're analyzing, regardless of whether it's related to any feature or not.
...
<<<<<<< Updated upstream
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=8, fig.height=4.5} 
data <- data.frame(feature_sentiment_data)

ggplot(data, aes(x=avg_sentiment, group=product, fill=product)) +
  ...
  #scale_x_continuous(breaks=as.numeric(c("-2", "-1", "0", "1", "2")))
=======
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=8, fig.height=4.5}
...
<!-- SPEAKER NOTES
  ...
  ##Apple
    ...
    From Apple's persepctive we would recommend the following:
      ...
      The iPhone has a very positive sentiment in both Russia and the USA but is lacking in Austrailia, Canada and South America which appear to have a higher sentiment towards Samsung. These location may be key for Apple to increase its sales by converting Samsung users to Apple user.      
