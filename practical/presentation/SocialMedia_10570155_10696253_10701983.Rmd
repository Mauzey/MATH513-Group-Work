...
<!--- SPEAKER NOTES:
  ...
  The normal twitter function allowed us to pull data from the last 6-9 days. We decided that just after the release date of the phones would be the best time to examine the public opinion.

  Due to the limitations of this search, we utilised the full archive premium feature to pull tweets relating to the S20. 
  
  However there was a mistake that led to the tweets being pulled a little earlier than the release date as seen by the red line on the graph. 
  
  we pulled as much data as we could over the last 30 days utilising the 30 day premium feature.
  ...
  From the graph, you can see that we managed to gather a reasonable amount of tweets over the month of November for both.

  You can also see the tweets for the S20 shown in red at the top whose release date was much earlier in the year where we used the Full Archive function. However
...
#df=data.frame(c(as.POSIXct(as.Date("2020-03-06")),
                    as.POSIXct(as.Date("2020-10-02")), 
                    as.POSIXct(as.Date("2020-10-23"))))
...
tweets %>% group_by(product, is_retweet) %>%
  ...
  #geom_vline(xintercept = as.POSIXct(as.Date("2020-03-06")))
  #geom_vline(mapping=aes(xintercept=df), color="black") 
```
...
For sentiment analysis we used R package 'sentimentr' which instead of calculating
sentiment scores for each word, calculates average sentiment for a group of sentences which would be more suitable for our case since technically we're analyzing the feedback (in form of tweets, each tweet consists of 1 or more whole sentences) on each feature for each of the 3 products.

This slide demonstrates the overall sentiment for each product. As you can see,
Galaxy s20 FE has the highest mean sentiment. Even though we're comparing the latest
2 products - iphone12 and galaxy s20 fe, we included sentiment for s20 as well in 
order to demonstrate how much it has improved. Which is to be expected
since the whole idea behind S20 FE is that Samsung took their customer's 
feedback into consideration and implemented some minor and major changes for features like display, price, battery.
...
Samsung's improvement of particular features which gathered feedback from customers who purchased S20 and S20 Ultra can be clearly seen from the following boxplots that
demonstrate sentiments for products of both Samsung and Apple grouped by features. We can observe that the sentiment for all the S20 FE's features except for camera, is higher than of S20, not mentioning that it's higher than of iphone12 in display, price and battery categories. 
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.align="center", fig.cap="", fig.show='hold', out.width='40%'}
...
<!---
!!!SOMEONE PLEASE PUT IT SOMEWHERE IN ANY SLIDE, THIS IS T-TEST
--->

```{r include=FALSE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=8, fig.height=4.5} 
