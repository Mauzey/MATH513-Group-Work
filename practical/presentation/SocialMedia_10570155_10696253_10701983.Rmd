...
<!--- SPEAKER NOTES: - WILL
  ...
  We came to the conclusion that it was better to use hashtags instead of pulling data from profiles, either business or individual. This was to ensure we would get a wide range of opinions for a fairer analysis of the public's sentiment.
...
tweets %>% group_by(product, is_retweet) %>%
  ...
  geom_vline(xintercept = as.POSIXct(as.Date("2020-03-06")), colour = "red") +
  geom_vline(xintercept = as.POSIXct(as.Date("2020-10-02")), colour = "green") +
  geom_vline(xintercept = as.POSIXct(as.Date("2020-10-23")), colour = "blue") +
  facet_grid(product ~ .)
```
...
<!-- SPEAKER NOTES - Alex
  ...
  This analysis shows us three main things:
    ...
    where there are darker areas for competitors of a certain product, extra attention should be taken in those regions to improve brand awareness and public sentiment   
