...
<!--- SPEAKER NOTES:
  ...
  - We removed duplicate entries for both the tweet and user data
    ...
    - Duplicate users were collected due to individual users posting multiple tweets
      - Started with about 110,000 users (one for each tweet), that went down to roughly 35,000 unique users
  ...
  - Users were marked as potential bots based on:
    ...
    - Whether the user's bio identified them as a bot, which was also through the use of keywords

  - The raw user locations varied due to twitter allowing custom locations:
    - We used the 'country code' package to extract countries in order to make use of geolocational information
    
  - Tweets were marked as potential spam based on:
    - Whether the tweet text identified it as spam, through the use of keywords
    - Whether the author of the tweet had been previously marked as a bot
  
  - We extracted product features from each tweet's text:
    - These features were: The product's Display, Camera, Battery, Price, and 5G Capabilities
  
  - A sentiment score was calculated for each tweet, by computing a sentiment for each word in the tweet,
...
<!--- SPEAKER NOTES:
  ...
  - Table 2 shows a summary of the user data:
    ...
    - Less than 1% of users were marked as bots. The actual amount of bots is 155
