...
<!--- SPEAKER NOTES - WILL
  ...
  Hi, today we will be discussing the result from the strategic analysis of Twitter data. 
  ...
  determine if there was any improvement in sentiment from the S20 to the S20FE from the public's viewpoint. 

  We utilised RStudio in combination with the 'rTweet' package to extract and analyse the data from the Twitter Developer API.

  To allow us to work professionally and on various parts of the project as a group, we decided to break up the code into 
  sections and utilise GitHub to keep track on the project.
...
<!--- SPEAKER NOTES: - WILL
  ...
  media platforms that has not limited the data provided through APIs (Ahemd, Dr. Wasim (2019))
  
  Twitter data can provide insights into the public's opinions and combined with the openness and the generous rate limiting of 
  Twitter’s API, it can produce powerful results. (Young, Michelle (2017))
  ...
  the official hashtags used by Samsung.
  ...
  We decided to look to the CEO of Apple, Tim Cook, as we thought he would most likely be using the official hashtag as deemed by
   Apple's marketing team, which led us to the hashtag '#iPhone12'.
...
<!--- SPEAKER NOTES - Will
  ...
  The normal twitter function allowed us to pull data from the last 6-9 days. We decided that just after the release date of the 
  phones would be the best time to examine the public's opinion.

  Due to the limitations of the general search, we utilised the full archive premium feature to pull tweets relating to the S20, 
  ...
  We learned from this and corrected our code and pulled as much data as we could for the other two phones using the premium 30 
  day function.
...
#### This is perhaps due to our tweets for the S20 being extracted before the release date.

      - This is perhaps due to the spammers focusing their efforts on newer, more relevant products in order to reach a wider audience
...
<!-- SPEAKER NOTES - Tania
  ...
  We decided to perform sentiment analysis both generally for each model 

###! MAYBE LEAVE THIS TILL NEXT SLIDE WHERE IT IS ON SCREEN
  and 
  for 5 of the most often mentioned features: Camera, 5G, Display, Price and Battery.
...
!!Overview of sentiment analysis##
  ...
  # NEED TO EXPLAIN THE MEAN AND BOX PLOT

  # MENTION WHAT THE IMPROVEMENTS
...
<!-- SPEAKER NOTES - Alex
  ...
  This global sentiment analysis shows us three main things:
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.height=3.7, fig.align='center'}
